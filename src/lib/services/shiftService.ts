 ...
 */
export async function saveWeeklyTemplates(userId: string, templates: WeeklyTemplate[]): Promise<void> {
  ...
  for (let day = 1; day <= lastDay; day++) {
    ...
    if (existingWishesMap.has(dateStr)) {
      ...
      if (extWish.isOverridden) {
        ...
        const isDefaultFree = extWish.type === 'free' && 
...
export async function getSubmitStatus(
  ...
  try {
    ...
    if (error.message?.includes('offline') || error.code?.includes('offline')) {
      console.warn('[Offline] Submit status check fell back to false (cache/offline):', error.message);
    } else {
 ...
 */
export async function getMonthlyConfirmedShifts(year: number, month: number): Promise<DailyShift[]> {
  const startDateStr = `${year}-${String(month).padStart(2, '0')}-01`;
  const lastDay = new Date(year, month, 0).getDate();
  const endDateStr = `${year}-${String(month).padStart(2, '0')}-${String(lastDay).padStart(2, '0')}`;
...
export async function backupToNotion(dailyShift: DailyShift): Promise<boolean> {
  ...
  try {
    ...
    if (staffIdList.length === 0) {
      console.log(`[Notion Backup] No assignments on ${dailyShift.date}. Clearing Notion database entries for this date.`);
    ...
    if (!response.ok) {
      ...
      console.warn('[Notion Confirmed Backup Warning] Notion sync endpoint returned non-ok:', response.status, errData.error || '');
      return false;
    }

    const resData = await response.json();
    if (resData.success) {
      console.log(`[Notion Confirmed Backup Success] Synchronized ${resData.count} records to Notion.`);
      return true;
    } else {
      console.warn('[Notion Confirmed Backup Warning] Notion sync failed in server handler:', resData.error);
      return false;
    }
  } catch (err) {
...
export async function backupWishesToNotion(
  ...
  try {
    ...
    if (!response.ok) {
      const errData = await response.json().catch(() => ({}));
      console.warn('[Notion Wishes Backup Warning] Notion sync returned non-ok status:', response.status, errData.error || '');
      return false;
    }
    
    const resData = await response.json();
    if (resData.success) {
      console.log(`[Notion Wishes Backup Success] Automatically synced ${resData.count} wishes to Notion.`);
      return true;
    } else {
      console.warn('[Notion Wishes Backup Warning] Notion sync failed in server handler:', resData.error);
      return false;
    }
  } catch (err) {
    console.warn('[Notion Wishes Backup Warning] Failed to fetch Notion sync API endpoint (might be offline or server not ready):', err);
    return false;
  }
}
  ...
  try {
    ...
    return snapshot.docs.map((d) => {
      ...
      return {
        ...
        targetIncomeMin: (data.targetIncomeMin !== undefined && data.targetIncomeMin !== null && data.targetIncomeMin > 0) ? data.targetIncomeMin : 40000,
        targetIncomeMax: (data.targetIncomeMax !== undefined && data.targetIncomeMax !== null && data.targetIncomeMax > 0) ? data.targetIncomeMax : 40000,
