 ...
 */
function getConsecutiveWorkingDays(staffId: string, targetDateStr: string, monthlyShifts: DailyShift[]): number {
...
export function parseDayOfWeek(val: any): number | null {
  ...
  if (typeof val === 'string') {
    ...
    const engIdx = ['sunday', 'monday', 'tuesday', 'wednesday', 'thursday', 'friday', 'saturday'].indexOf(clean);
...
export function normalizeWish(wish: any): { date: string; type: 'ng' | 'specific' | 'free'; startTime: string; endTime: string; isSubmitted: boolean; isOverridden: boolean } {
  if (!wish) {
    return { date: '', type: 'free', startTime: '09:45', endTime: '20:15', isSubmitted: false, isOverridden: false };
  }

  const typeStr = String(wish.type || '').trim().toLowerCase();
  const startStr = String(wish.startTime || '').trim().toLowerCase();
  const endStr = String(wish.endTime || '').trim().toLowerCase();
  ...
  // 固定枠IDの処理
  if (startStr === 'frame1') {
    return { date, type: 'specific', startTime: '09:45', endTime: '15:00', isSubmitted, isOverridden };
  }
  if (startStr === 'frame2') {
    return { date, type: 'specific', startTime: '15:00', endTime: '20:15', isSubmitted, isOverridden };
  }
...
export function generateDailyShift(
  ...
  const nonExcludedStaffs = sortedStaffs.filter(item => {
    const s = item.staff;
    const maxIncomeCap = s.role === 'employee' ? (Number(s.target_monthly_income || s.targetIncomeMax) || 250000) : Math.min(50000, Number(s.target_monthly_income || s.targetIncomeMax) || 50000);
    ...
    TIME_SLOTS.forEach(slot => {
      if (isSlotInInterval(slot, assign.startTime, assign.endTime)) {
        slots[slot].push({
          ...
          role: (assign.staff.age_group || assign.staff.role) === 'minor' ? 'minor' : (assign.staff.role === 'employee' ? 'employee' : 'adult'),
          area: 'cafe'
        });
  ...
  if (hasUnicesEvent) {
    ...
    const unicesCandidates = staffs.filter(s => {
      ...
      return isSlotInInterval(event.startTime, wish.startTime, wish.endTime) &&
      ...
      TIME_SLOTS.forEach(slot => {
        if (isSlotInInterval(slot, event.startTime, event.endTime)) {
          slots[slot].push({
            ...
            role: (unicesStaff!.age_group || unicesStaff!.role) === 'minor' ? 'minor' : (unicesStaff!.role === 'employee' ? 'employee' : 'adult'),
            area: 'unices'
          });
 ...
 */
export function validateDailyShift(
  ...
  TIME_SLOTS.forEach((slot) => {
    ...
    assignments.forEach((assignment) => {
      if (assignment.role === 'minor') {
        minorWorkSlotsCount[assignment.staffId] = (minorWorkSlotsCount[assignment.staffId] || 0) + 1;
      }
