...
class AuthState {
            ...
            try {
              ...
              if (snap.exists()) {
                ...
                this.user = {
                  ...
                  name: isKharu ? '管理者 (kharu)' : (data.name || firebaseUser.displayName || '未設定'),
                ...
                const newSession: UserSession = {
                  uid: firebaseUser.uid,
                  email: firebaseUser.email,
                  name: isKharu ? '管理者 (kharu)' : (firebaseUser.displayName || firebaseUser.email?.split('@')[0] || '新規スタッフ'),
              ...
              this.user = {
                uid: firebaseUser.uid,
                email: firebaseUser.email,
                name: isKharu ? '管理者 (kharu)' : (firebaseUser.displayName || firebaseUser.email?.split('@')[0] || 'ユーザー'),
                role: isKharu ? 'employee' : 'adult',
                isAdmin: isKharu,
                notion_person_id: ''
              };
  ...
  async loginWithEmail(email: string, pass: string): Promise<void> {
    ...
    try {
      ...
      const isConfigError = e.code === 'auth/configuration-not-found' || e.message?.includes('configuration-not-found');
      const isOfflineError = e.message?.includes('offline') || e.code?.includes('network-request-failed');
      ...
      if (isConfigError || isOfflineError || typeof window !== 'undefined') {
        ...
        if (lowerEmail.includes('admin') || lowerEmail.includes('employee') || lowerEmail.includes('sato') || lowerEmail.includes('tanaka') || lowerEmail === 'kharu2514@gmail.com') {
          role = 'employee';
        } else if (lowerEmail.includes('minor') || lowerEmail.includes('ito') || lowerEmail.includes('watanabe')) {
          role = 'minor';
        }
   ...
   * 新規スタッフ登録
   */
  async registerWithEmail(email: string, pass: string, name: string, role: 'employee' | 'adult' | 'minor'): Promise<void> {
    this.loading = true;
    try {
      ...
      if (typeof window !== 'undefined') {
        ...
        const finalRole = isKharu ? 'employee' : role;
        const finalIsAdmin = isKharu ? true : (role === 'employee');
        const finalName = isKharu ? '管理者 (kharu)' : `${name} (${finalRole === 'employee' ? '社員' : finalRole === 'adult' ? '成人' : '未成年'})`;
        ...
        localStorage.setItem('ipo_mock_session', JSON.stringify(mockUser));
        this.loading = false;
        console.log('[Auth] Registered and logged in successfully using local simulation:', mockUser);
        return;
      }
