...
export const POST: RequestHandler = async ({ request }) => {
	try {
		...
		if (!NOTION_API_TOKEN || !NOTION_CONFIRMED_DATABASE_ID) {
			console.warn(
				'[Notion Confirmed API] NOTION_API_TOKEN or NOTION_CONFIRMED_DATABASE_ID is not configured in the .env file.'
			);
			return json(
				{ success: false, error: 'Notion credentials are not configured in .env file.' },
				{ status: 500 }
			);
		}
		...
		const queryResponse = await fetch(
			`https://api.notion.com/v1/databases/${NOTION_CONFIRMED_DATABASE_ID}/query`,
			{
				method: 'POST',
				headers: {
		...
		if (queryResponse.ok) {
			...
			if (existingPages.length > 0) {
				console.log(
					`[Notion Confirmed API] Archiving ${existingPages.length} existing pages for date ${date} to prevent duplicates...`
				);
				// 既存のページを並行してアーカイブ化（削除）
				await Promise.all(
					existingPages.map(async (page: any) => {
						try {
		...
		console.log(
			`[Notion Confirmed API] Inserting ${records.length} new confirmed shift records...`
		);
		const insertResults = await Promise.all(
			records.map(async (rec: any) => {
				try {
					// エリア名の決定
					const areaName =
						rec.areas.includes('cafe') && rec.areas.includes('unices')
							? '混合'
					...
					const properties: any = {
						シフト名: {
							title: [{ text: { content: `[確定] ${date} ${rec.name}` } }]
						},
						日付: {
							date: { start: date }
						},
						ユーザーID: {
							rich_text: [{ text: { content: rec.uid || rec.staffId } }]
						},
						開始時間: {
							rich_text: [{ text: { content: rec.startTime || '' } }]
						},
						終了時間: {
							rich_text: [{ text: { content: rec.endTime || '' } }]
						},
						エリア: {
					...
					if (rec.notion_person_id) {
						properties['スタッフ名'] = {
							people: [{ id: rec.notion_person_id }]
						};
					} else {
						console.log(
							`[Notion Confirmed API] Staff ${rec.name} has no notion_person_id. Skipping Person attribute.`
						);
					}
					...
					if (!response.ok) {
						...
						console.warn(
							`[Notion Confirmed API] Failed to insert row for ${rec.name}: ${response.status} ${errText}`
						);
						return false;
					}
		...
		console.log(
			`[Notion Confirmed API] Successfully synced ${successCount}/${records.length} shift records to Notion.`
		);
