...
export const POST: RequestHandler = async ({ request }) => {
	try {
		...
		if (!NOTION_API_TOKEN || !NOTION_WISHES_DATABASE_ID) {
			console.warn(
				'[Notion API Server] NOTION_API_TOKEN or NOTION_WISHES_DATABASE_ID is not configured in the .env file.'
			);
			return json(
		...
		console.log(
			`[Notion API Server] Querying existing wishes for ${userId} between ${startDateStr} and ${endDateStr}...`
		);

		const queryResponse = await fetch(
			`https://api.notion.com/v1/databases/${NOTION_WISHES_DATABASE_ID}/query`,
			{
				method: 'POST',
				headers: {
		...
		if (queryResponse.ok) {
			...
			if (existingPages.length > 0) {
				console.log(
					`[Notion API Server] Archiving ${existingPages.length} existing pages to prevent duplicates...`
				);
					...
					const response = await fetch('https://api.notion.com/v1/pages', {
						...
						body: JSON.stringify({
							...
							properties: {
								...
								希望区分: {
									select: {
										name: w.type === 'specific' ? '特定時間' : w.type === 'ng' ? 'NG' : 'おまかせ'
									}
								},
								開始時間: {
									rich_text: [{ text: { content: w.startTime || '' } }]
								},
								終了時間: {
									rich_text: [{ text: { content: w.endTime || '' } }]
								},
								個別上書き: {
		...
		console.log(
			`[Notion API Server] Successfully synced ${successCount}/${wishes.length} wishes to Notion.`
		);
