...
<script lang="ts">
	...
	// 詳細ポップアップ用のステート
	let detailModalDay = $state<{ dateStr: string; day: number; dailyShift?: DailyShift } | null>(
		null
	);
	...
	function getStaffWorkingHoursForDay(
		dailyShift: DailyShift | undefined | null,
		staffsList: Staff[]
	): StaffShiftTime[] {
	...
	function onShiftCellClick(cell: {
		dateStr: string;
		day: number;
		isEmpty: boolean;
	...
	let targetMid = $derived(
		targetMonthlyIncomeInput > 0
			? targetMonthlyIncomeInput
			: targetIncomeMax > 0
				? targetIncomeMax
				: 50000
	...
	let wageProgressPercent = $derived(
		targetMid > 0 ? Math.min(100, Math.round((expectedMonthlyWage / targetMid) * 100)) : 0
	);
	...
	async function saveCustomTime(dateStr: string, day: number) {
		...
		if (dayTemplates.length > 0) {
			...
			if (
				...
				template.startTime === finalStart &&
				template.endTime === finalEnd
			) {
				isOverridden = false;
			}
		}
...
>
	<div class="mx-auto max-w-4xl space-y-8 px-4 py-8">
		<!-- 上部: ヘッダー & ユーザー制御 -->
		<header
			class="flex flex-col items-start justify-between gap-4 border-b border-slate-200 pb-6 sm:flex-row sm:items-center"
		>
			<div class="space-y-1">
			...
			{#if authState.user?.isAdmin}
				<div
					class="flex items-center gap-3 rounded-full border border-slate-200 bg-white px-4 py-2 text-xs shadow-sm"
				>
					<div class="flex items-center gap-1.5 border-r border-slate-100 pr-3">
						<label
							for="demo-user-id"
							class="text-[10px] font-semibold tracking-wider text-slate-400 uppercase">User:</label
						>
						<input
							id="demo-user-id"
							type="text"
							...
							class="w-28 rounded-full border border-slate-200 bg-slate-50 px-3 py-1 text-xs text-slate-800 transition-all duration-300 focus:border-slate-400 focus:bg-white focus:outline-none"
						/>
					</div>
					...
					<div class="flex items-center gap-2">
						<label
							for="simulate-lock"
							class="cursor-pointer text-[10px] font-semibold text-slate-500">手動ロック</label
						>
						<input
							id="simulate-lock"
							type="checkbox"
							bind:checked={isSystemLocked}
							class="h-3.5 w-3.5 rounded border-slate-300 bg-slate-100 text-slate-800 focus:ring-slate-400 focus:ring-offset-0"
						/>
					</div>
				</div>
			{/if}
		...
		<nav
			class="mx-auto flex max-w-md justify-center rounded-2xl border border-slate-200/60 bg-slate-100 p-1 shadow-sm"
		>
			<button
				type="button"
				onclick={() => (activeTab = 'shifts')}
				class="flex flex-1 items-center justify-center gap-1.5 rounded-xl px-3 py-2 text-xs font-bold transition-all duration-300
          {activeTab === 'shifts'
		...
		<main class="space-y-6">
			...
			{#if activeTab === 'shifts'}
				<section
					class="animate-in fade-in space-y-6 rounded-3xl border border-slate-200/80 bg-white p-6 shadow-[0_8px_30px_rgb(0,0,0,0.015)] duration-300 sm:p-8"
				>
					<!-- 前後半ステータスバッジ (画面上部) -->
					<div class="grid grid-cols-2 gap-4">
						<div
							class="flex flex-col justify-between rounded-2xl border border-slate-200/60 bg-slate-50 p-3 transition duration-300"
						>
							...
							<div class="mt-1.5 flex items-center justify-between">
								...
								<span
									class="rounded-full border border-emerald-100 bg-emerald-50 px-2.5 py-0.5 text-[10px] font-bold text-emerald-600"
								>
									確定済み ✅
								</span>
							</div>
						...
						>
							...
							<div class="mt-1.5 flex items-center justify-between">
								...
								<span
									class="rounded-full border border-amber-100 bg-amber-50 px-2.5 py-0.5 text-[10px] font-bold text-amber-600"
								>
									調整中 ⏳
								</span>
							</div>
					...
					<div
						class="flex flex-col items-start justify-between gap-4 border-b border-slate-100 pb-4 sm:flex-row sm:items-center"
					>
						<div>
						...
						>
							<!-- 自分のみ・全員の切り替えコントロール -->
							<div
								class="flex items-center gap-0.5 rounded-full border border-slate-200/60 bg-slate-100 p-0.5 text-xs shadow-sm"
							>
								<button
									type="button"
									onclick={() => (showAllShifts = false)}
									class="flex items-center gap-1 rounded-full px-3 py-1.5 font-bold transition-all duration-300
                    {!showAllShifts
										? 'bg-white text-slate-900 shadow-sm ring-1 ring-slate-200/50'
                    ...
                    {showAllShifts
							...
							<!-- 提出対象年月表示 -->
							<div
								class="rounded-full border border-indigo-100 bg-indigo-50 px-3.5 py-1 font-mono text-[11px] font-extrabold tracking-wider text-indigo-700"
							>
					...
					<!-- 曜日見出し -->
					<div
						class="grid grid-cols-7 gap-1 pb-2 text-center text-[10px] font-bold tracking-widest text-slate-400 uppercase sm:gap-2"
					>
					...
					<div class="grid grid-cols-7 gap-1 sm:gap-2">
						{#each shiftCalendarCells as cell}
							{#if cell.isEmpty}
								<div
									class="aspect-[4/3] rounded-2xl border border-transparent bg-slate-50/10 sm:aspect-square"
								></div>
							{:else}
								...
								<div
									...
									class="group hover:border-slate-450 relative flex min-h-[110px] cursor-pointer flex-col justify-between overflow-hidden rounded-2xl border p-2 shadow-sm transition-all duration-300 ease-in-out hover:scale-[1.02] hover:shadow-md sm:min-h-[140px]
                    {hasMe
                    ...
                    "
										...
										{#if hasMe}
											<span
												class="flex animate-pulse items-center justify-center gap-0.5 rounded-full bg-indigo-600 px-1.5 py-0.5 text-[8px] font-extrabold text-white shadow-sm"
											>
												<span>✨</span>
												<span>出勤</span>
											</span>
									...
									<div
										class="mt-1.5 flex max-h-[72px] flex-grow scrollbar-none flex-col gap-1 overflow-y-auto pr-0.5 select-none sm:max-h-[100px]"
									>
										{#if !showAllShifts}
											{@const myAssign = workingStaff.find((assign) => assign.staffId === userId)}
											{#if myAssign}
												<div
													class="animate-in fade-in zoom-in-95 flex h-full min-h-[50px] flex-col items-center justify-center gap-1 rounded-2xl border border-indigo-700 bg-indigo-600 px-1.5 py-2.5 text-[9px] font-extrabold text-white shadow-[0_2px_8px_rgba(99,102,241,0.25)] ring-1 ring-indigo-400 duration-200 sm:text-[10px]"
												>
													<span class="text-[8px] tracking-widest uppercase opacity-90"
														>出勤時間</span
													>
													<span
														class="rounded-full bg-indigo-800/40 px-2 py-0.5 font-mono text-xs font-bold"
														>{myAssign.timeRangeStr}</span
													>
												</div>
											{:else}
												<div
													class="text-slate-355 flex h-full items-center justify-center py-4 text-[9px] font-semibold italic"
												>
													お休み ☕
												</div>
											{/if}
										{:else if workingStaff.length === 0}
											<div
												class="text-slate-355 flex h-full items-center justify-center py-2 text-[9px] font-medium italic"
											>
												休み ☕
											</div>
										{:else}
											{#each workingStaff.slice(0, 3) as assign}
												...
												<div
													class="truncate rounded border px-1.5 py-0.5 text-[8px] font-bold tracking-tight transition duration-300 sm:text-[9px]
                              {isSelf
														? 'border-indigo-700 bg-indigo-600 text-white shadow-sm ring-1 ring-indigo-400'
														: 'border-slate-200/80 bg-slate-50 text-slate-700'}"
												>
													{#if assign.role === 'employee'}[社員]{:else if assign.role === 'adult'}[成人]{:else}[未成]{/if}
													{assign.staffName.split(' ')[0]}
													<span class="font-mono opacity-90">({assign.timeRangeStr})</span>
												</div>
											{/each}
											{#if workingStaff.length > 3}
												<div
													class="py-0.5 text-center text-[8px] font-extrabold text-slate-400 sm:text-[9px]"
												>
					...
					<!-- 凡例 -->
					<div
						class="flex flex-wrap items-center gap-4 border-t border-slate-100 pt-4 font-sans text-[10px] font-medium text-slate-400"
					>
						<div class="flex items-center gap-1.5">
							<span
								class="inline-block h-3.5 w-3.5 rounded-lg bg-indigo-600 shadow-sm ring-2 ring-indigo-200/50"
							></span>
							<span class="font-bold text-slate-700">自分のシフト（グロウ強調）</span>
						</div>
						<div class="flex items-center gap-1.5">
							<span class="inline-block h-3.5 w-3.5 rounded-lg border border-slate-200 bg-slate-50"
							></span>
							<span class="text-slate-600">他のメンバーの出勤</span>
						</div>
						<div class="flex items-center gap-1.5">
							<span
								class="text-slate-350 flex h-3.5 w-3.5 items-center justify-center rounded-lg border border-slate-100 bg-white text-[8px] italic"
								>☕</span
							>
				...
				>
					...
					>
						<div class="flex items-center space-x-3">
							<h2 class="text-lg font-bold tracking-tight text-slate-800">希望カレンダー</h2>
							<!-- 提出対象年月表示 -->
							<div
								class="rounded-full border border-indigo-100 bg-indigo-50 px-3.5 py-1 font-mono text-[11px] font-extrabold tracking-wider text-indigo-700"
							>
						...
						<!-- 自動締め切り設定表示（25日締め自動化） -->
						<div
							class="flex items-center gap-3 rounded-full border border-slate-200/60 bg-slate-50 px-4 py-2 font-sans text-[10px] text-slate-500"
						>
							<span class="font-bold tracking-wider text-slate-400 uppercase">締切日時:</span>
							<span class="font-mono font-bold text-slate-700"
								>{deadlineYear}/{String(deadlineMonth).padStart(2, '0')}/25 23:59</span
							>

							<!-- 個別解除バイパストグル -->
							<div class="flex items-center gap-1.5 border-l border-slate-200 pl-3">
								<label for="override-lock" class="font-bold text-amber-600">個別解除</label>
								<input
									id="override-lock"
									type="checkbox"
									bind:checked={userIsUnlocked}
									onchange={() => toggleUnlockBypass(userIsUnlocked)}
									class="border-slate-350 h-3.5 w-3.5 rounded bg-white text-amber-500 focus:ring-amber-400"
								/>
							</div>
						</div>
					</div>
					...
					<div
						class="flex flex-col gap-6 space-y-4 rounded-3xl border border-indigo-100/70 bg-gradient-to-r from-indigo-50/60 to-teal-50/30 p-5 shadow-sm md:flex-row md:items-start md:justify-between"
					>
						<div class="flex-1 space-y-3">
							<div class="space-y-1">
								...
								<p class="text-[10px] font-medium text-slate-500">
									今月のシフト配置における目標希望金額を ¥5,000 単位で設定してください。
								</p>
							</div>
							<div
								class="flex max-w-[220px] items-center justify-between gap-2 rounded-2xl border border-slate-200/80 bg-white/90 px-4 py-2 shadow-xs backdrop-blur-sm transition-colors hover:border-slate-300"
							>
						...
						<div class="flex-1 space-y-3">
							<div class="space-y-1">
								...
								<p class="text-[10px] font-medium text-slate-500">
									これ以上絶対に稼ぎたくない月収の上限（未設定時は70,000円）です。
								</p>
							</div>
							<div
								class="flex max-w-[220px] items-center justify-between gap-2 rounded-2xl border border-slate-200/80 bg-white/90 px-4 py-2 shadow-xs backdrop-blur-sm transition-colors hover:border-slate-300"
					...
					<div
						class="flex flex-col justify-between gap-3 rounded-2xl border p-4 shadow-xs transition-all duration-300 sm:flex-row sm:items-center
            {isDeadlinePassed
							? 'animate-pulse border-amber-100 bg-amber-50/75 text-amber-800'
							: isLocked
					...
					>
						<div class="flex items-center gap-3">
							...
							<div>
								<p class="flex flex-wrap items-center gap-1.5 text-xs font-extrabold">
									<span>希望提出締め切り日時:</span>
									<span
										class="rounded-full border bg-white/90 px-3 py-0.5 font-mono text-xs shadow-xs
                    {isDeadlinePassed
											? 'border-amber-200 font-extrabold text-amber-600'
											: isLocked
								...
								<p class="mt-1 text-[9px] font-bold opacity-90">
									{#if isDeadlinePassed}
						...
						<!-- ステータスラベル -->
						<span
							class="self-start rounded-full px-4.5 py-1.5 text-center text-[10px] font-extrabold tracking-wider uppercase shadow-xs sm:self-center
              {isDeadlinePassed
								? 'bg-amber-500 text-white'
								: isLocked
									? 'animate-pulse bg-rose-500 text-white'
									: userIsUnlocked
										? 'bg-amber-500 font-extrabold text-white'
					...
					{#if isLoading}
						<div class="flex items-center justify-center py-32">
							<div class="h-8 w-8 animate-spin rounded-full border-b-2 border-slate-800"></div>
						</div>
					{:else}
						<!-- 曜日見出し -->
						<div
						...
						>
							{#each dayNames as name, i}
								<span class={i === 0 ? 'text-rose-400' : i === 6 ? 'text-blue-400' : ''}
									>{name}</span
								>
							{/each}
						</div>
						...
						<div class="grid grid-cols-7 gap-1 sm:gap-2">
							{#each calendarCells as cell}
								{#if cell.isEmpty}
									<div
										class="aspect-square rounded-2xl border border-transparent bg-slate-50/10"
									></div>
								{:else}
                    ...
                    "
                      ...
                      "
											...
											{#if wish?.isOverridden}
												<span
													class="h-1.5 w-1.5 rounded-full bg-amber-400 ring-2 ring-amber-100"
													title="個別修正あり"
												></span>
											{/if}
										</div>
										...
										<div class="flex w-full flex-grow flex-col items-center justify-center py-1">
											{#if editingDateStr === cell.dateStr}
												...
												<input
													...
													class="border-slate-350 w-full rounded-xl border bg-white px-1 py-1 text-center font-mono text-[9px] font-bold text-slate-800 transition focus:ring-2 focus:ring-indigo-400 focus:outline-none"
												/>
										...
										{#if !isLocked}
											<button
												...
												class="absolute right-1 bottom-1 flex h-4.5 w-4.5 items-center justify-center rounded-full border border-slate-200/65 bg-slate-100/90 text-[9px] text-slate-500 opacity-60 shadow-sm transition duration-200 group-hover:opacity-100 hover:bg-slate-200 hover:text-slate-700 focus:opacity-100 sm:opacity-0"
												title="メニューを開く"
											>
												⋮
											</button>
										...
										{#if activeMenuDate === cell.dateStr && !isLocked}
											...
											<div
												onclick={(e) => e.stopPropagation()}
												class="animate-in fade-in zoom-in-95 absolute right-1 bottom-6 z-50 flex min-w-[130px] flex-col rounded-2xl border border-slate-200 bg-white py-1.5 text-left shadow-xl duration-150"
											>
												<button
													type="button"
													onclick={(e) => {
														e.stopPropagation();
														applyStampById(cell.dateStr, cell.day, 'free');
														activeMenuDate = null;
													}}
													class="text-slate-750 flex w-full items-center gap-1.5 px-3 py-1 text-[9px] font-bold transition hover:bg-slate-50"
												>
												...
												<button
													...
													onclick={(e) => {
														e.stopPropagation();
														applyStampById(cell.dateStr, cell.day, 'ng');
														activeMenuDate = null;
													}}
													class="text-slate-750 flex w-full items-center gap-1.5 px-3 py-1 text-[9px] font-bold transition hover:bg-slate-50"
												>
												...
												<button
													...
													onclick={(e) => {
														e.stopPropagation();
														applyStampById(cell.dateStr, cell.day, 'fixed1');
														activeMenuDate = null;
													}}
													class="text-slate-750 flex w-full items-center gap-1.5 px-3 py-1 text-[9px] font-bold transition hover:bg-slate-50"
												>
												...
												<button
													...
													onclick={(e) => {
														e.stopPropagation();
														applyStampById(cell.dateStr, cell.day, 'fixed2');
														activeMenuDate = null;
													}}
													class="text-slate-750 flex w-full items-center gap-1.5 px-3 py-1 text-[9px] font-bold transition hover:bg-slate-50"
												>
												...
												<div class="my-1 h-[1px] bg-slate-100"></div>
												<button
													type="button"
													onclick={(e) => {
														e.stopPropagation();
													...
													class="text-indigo-650 flex w-full items-center gap-1.5 px-3 py-1 text-[9px] font-bold transition hover:bg-indigo-50/50"
												>
													<span class="text-xs">📝</span>
													<span>カスタム時間を設定...</span>
												</button>
			...
			{#if activeTab === 'templates'}
				<div class="animate-in fade-in space-y-6 duration-300">
					<!-- 上部: 曜日パターン（スタンプ）パレット -->
					<section
						class="space-y-5 rounded-3xl border border-slate-200/80 bg-white p-6 shadow-[0_8px_30px_rgb(0,0,0,0.015)]"
					>
						<div
							class="flex flex-col items-start justify-between gap-2 border-b border-slate-100 pb-3 sm:flex-row sm:items-center"
						>
							<div>
								<h2
									class="flex items-center gap-1.5 text-base font-bold tracking-tight text-slate-800"
								>
									<span>🎨</span>
									<span>曜日基本パターンパレット</span>
								</h2>
								<p class="mt-0.5 text-[10px] text-slate-400">
									曜日カードに適用したいパターンを選択してアクティブにしてください。
								</p>
							</div>
							<span
								class="rounded-full border border-indigo-100 bg-indigo-50 px-3 py-1 text-[9px] font-extrabold tracking-wider text-indigo-600 uppercase shadow-sm"
							>
								パターンを選んで、下の曜日カードを1タップ！
							</span>
						</div>

						<div class="grid grid-cols-2 gap-3 select-none md:grid-cols-4">
							{#each templateStamps as stamp}
								<button
									type="button"
									onclick={() => (activeTemplateStampId = stamp.id)}
									disabled={isLocked}
									class="flex flex-col items-center justify-center gap-2 rounded-2xl border-2 px-4 py-4 text-center font-sans text-xs font-bold shadow-sm transition-all duration-300
                    {isLocked
										? 'cursor-not-allowed border-slate-200 bg-slate-100/50 text-slate-400 opacity-50'
										: 'cursor-pointer hover:scale-[1.02] active:scale-95'}
                    {activeTemplateStampId === stamp.id && !isLocked
										? 'scale-[1.01] border-slate-900 bg-slate-900 text-white shadow-md ring-4 ring-slate-200/50'
										: `${stamp.colorClass} border-slate-200/85 hover:border-slate-400`}"
								>
									<span class="text-xl">{stamp.icon}</span>
									<span
										class="font-extrabold text-slate-800 {activeTemplateStampId === stamp.id &&
										!isLocked
											? 'text-white'
											: ''}">{stamp.label}</span
									>
									{#if stamp.id === 'custom'}
										<span
											class="mt-0.5 rounded-full border px-2 py-0.5 font-mono text-[9px] opacity-90 {activeTemplateStampId ===
												stamp.id && !isLocked
												? 'border-slate-700 bg-slate-800 text-amber-300'
												: 'border-amber-200/50 bg-white text-amber-600'}"
										>
						...
						{#if activeTemplateStampId === 'custom'}
							<div
								class="animate-in slide-in-from-top-2 space-y-4 rounded-3xl border border-amber-200/60 bg-amber-50/50 p-5 duration-300 select-none"
							>
								<div class="text-amber-850 flex items-center justify-between text-xs font-bold">
									...
									<span>
										設定範囲: <span
											class="rounded-full border border-amber-300 bg-white px-3 py-1 font-mono text-sm text-amber-700 shadow-sm"
											>{minutesToTimeStr(customStartMinutes)} 〜 {minutesToTimeStr(
												customEndMinutes
											)}</span
										>
									</span>
								...
								<div class="space-y-4 pt-2">
									...
									<div class="space-y-1">
										<div class="flex justify-between text-[10px] font-bold text-slate-400">
											<span
												>開始時間: <span class="font-mono font-bold text-amber-600"
													>{minutesToTimeStr(customStartMinutes)}</span
												></span
											>
											<span>最小: 09:45</span>
										</div>
										<input
											...
											disabled={isLocked}
											class="h-1.5 w-full cursor-pointer appearance-none rounded-lg bg-slate-200 accent-amber-500 focus:outline-none disabled:cursor-not-allowed disabled:opacity-50"
										/>
									</div>
									...
									<div class="space-y-1">
										<div class="flex justify-between text-[10px] font-bold text-slate-400">
											<span
												>終了時間: <span class="font-mono font-bold text-amber-600"
													>{minutesToTimeStr(customEndMinutes)}</span
												></span
											>
											<span>最大: 20:15</span>
										</div>
								...
								<p class="text-center text-[9px] font-bold text-amber-600/80">
					...
					>
						<div class="flex items-center justify-between border-b border-slate-100 pb-2">
							<div>
								...
								<p class="mt-0.5 text-[10px] text-slate-400">
									曜日カードをクリックして、選択中のパターンを1タップ割り当てします。
								</p>
							</div>
						</div>

						<div class="grid grid-cols-2 gap-3 select-none sm:grid-cols-7">
							{#each Array(7) as _, index}
								{@const dayOfWeekNum = (index + 1) % 7}
								<!-- 月=1, 火=2, ... 土=6, 日=0 -->
								{@const dayName = dayNames[dayOfWeekNum]}
								{@const currentTemplate = weeklyTemplates.find(
									(t) => Number(t.dayOfWeek) === dayOfWeekNum
								)}
								{@const isNg = currentTemplate?.type === 'ng'}
								{@const isSpecific = currentTemplate?.type === 'specific'}
								{@const isFixed =
									isSpecific &&
									currentTemplate?.startTime === '09:45' &&
									currentTemplate?.endTime === '15:00'}
								...
								<div
									...
									onkeydown={(e) =>
										(e.key === 'Enter' || e.key === ' ') &&
										!isLocked &&
										applyStampToWeekday(dayOfWeekNum)}
									class="flex min-h-[150px] flex-col justify-between rounded-2xl border p-4 text-center transition-all duration-300 ease-in-out
                    {isLocked
										? 'cursor-not-allowed opacity-50'
										: 'cursor-pointer hover:scale-[1.03] hover:shadow-md active:scale-95'}
                    {isFree
										? 'hover:border-slate-450 border-slate-200 bg-slate-50 text-slate-500'
										: isNg
								...
								>
									<!-- カード上部: 曜日名 -->
									<span
										class="block border-b border-slate-200/50 pb-1.5 text-xs font-bold tracking-wider uppercase
                    {isFree
											? 'text-slate-700'
											: isNg
												? 'text-rose-750'
									...
									<!-- カード中部: ステータスアイコン＆表示 -->
									<div class="flex flex-grow flex-col items-center justify-center py-3">
										{#if isFree}
											<div
												class="animate-in zoom-in-95 flex flex-col items-center gap-1 duration-200"
											>
									...
									<!-- カード下部: 時間帯表記 -->
									<span
										class="block border-t border-slate-200/40 pt-1 font-mono text-[8px] font-bold opacity-90"
									>
										{#if isFree}
											09:45 〜 20:15
										{:else if isNg}
...
<div
	class="animate-in slide-in-from-bottom-5 fixed bottom-6 left-1/2 z-40 w-full max-w-xl -translate-x-1/2 px-4 duration-500 select-none"
>
	<div
		class="space-y-4 rounded-[28px] border border-slate-200/80 bg-white/96 p-5 shadow-[0_15px_40px_rgba(0,0,0,0.08)] backdrop-blur-md"
	>
		<!-- リアルタイム希望給料進捗バー -->
		<div class="space-y-1.5">
			<div class="flex items-center justify-between text-[10px] font-bold">
				<span class="flex items-center gap-1 tracking-wider text-slate-400 uppercase">
					<span>💰</span>
					<span>今月の想定希望月収</span>
				</span>
				<span class="text-slate-700">
					想定: <span class="text-xs font-extrabold text-slate-900"
						>¥{expectedMonthlyWage.toLocaleString()}</span
					>
			...
			<div class="h-2 w-full overflow-hidden rounded-full border border-slate-200/40 bg-slate-100">
				<div
					class="h-full rounded-full bg-gradient-to-r from-teal-400 to-indigo-600 shadow-[0_0_8px_rgba(99,102,241,0.3)] transition-all duration-500"
		...
		<!-- アクション＆提出ステータス -->
		<div class="flex items-center justify-between gap-4 border-t border-slate-100/60 pt-2">
			<div class="flex items-center gap-1.5 text-[10px] font-bold text-slate-500">
				<span
					class="h-2 w-2 rounded-full {isSyncingWishesNotion
			...
			<button
				...
				disabled={isLocked}
				class="disabled:bg-slate-150 flex cursor-pointer items-center gap-1.5 rounded-full bg-slate-900 px-6 py-2.5 text-xs font-extrabold text-white shadow-sm transition-all duration-300 ease-in-out hover:scale-[1.03] hover:bg-slate-800 active:scale-95 disabled:cursor-not-allowed disabled:text-slate-400"
			>
				<span
					>{isDeadlinePassed
						? '修正要望として提出する'
...
{#if isSavedToast}
	<div
		class="animate-in slide-in-from-top-4 fixed top-6 right-6 z-50 flex items-center gap-2.5 rounded-full border border-slate-800 bg-slate-900 px-5 py-3.5 text-white shadow-2xl duration-300"
	>
		<span class="text-sm">🎉</span>
		<div>
			<p class="text-xs font-bold">
				{isDeadlinePassed ? '修正要望を提出しました' : 'シフト希望を提出しました'}
			</p>
		</div>
	</div>
{/if}
...
{#if isDetailModalOpen && detailModalDay}
	...
	<div
		...
		class="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/20 p-4 backdrop-blur-md"
	>
		<div
			role="document"
			tabindex="-1"
			...
			class="w-full max-w-md space-y-5 rounded-[32px] border border-slate-200 bg-white p-6 shadow-[0_24px_50px_rgba(0,0,0,0.12)] focus:outline-none"
		>
			<!-- モーダルヘッダー -->
			<div class="flex items-start justify-between border-b border-slate-100 pb-2">
				<div>
					<h3 class="flex items-center gap-2 text-sm font-extrabold tracking-tight text-slate-800">
						<span>出勤メンバー一覧</span>
					</h3>
					<p class="mt-0.5 text-[10px] font-bold text-slate-400">
						日付: <span class="text-slate-800"
							>{targetMonth}月{detailModalDay.day}日 ({dayOfWeekName}曜日)</span
						>
					</p>
				</div>
				<button
					type="button"
					onclick={() => (isDetailModalOpen = false)}
					class="hover:text-slate-655 rounded-full bg-slate-50 p-1 text-lg font-bold text-slate-400 transition duration-300"
				>
					✕
				</button>
			</div>
			...
			<div class="max-h-[320px] space-y-3 overflow-y-auto pr-1">
				{#if workingStaff.length === 0}
					<div
						class="text-slate-450 flex flex-col items-center justify-center gap-2 py-10 text-center text-xs italic"
					>
				...
				{:else}
					{#each workingStaff as assign}
						{@const isSelf = assign.staffId === userId}
						<div
							class="flex items-center justify-between rounded-2xl border p-3 transition duration-300
                {isSelf
								? 'border-indigo-200 bg-indigo-50/50 shadow-[0_0_12px_rgba(99,102,241,0.1)] ring-1 ring-indigo-200/50'
								: 'border-slate-200/60 bg-slate-50'}"
						>
							<div class="flex items-center gap-2.5">
								<span
									class="rounded-full px-2 py-0.5 text-[8px] font-bold tracking-wider
                  {assign.role === 'employee'
								...
								<span class="text-xs font-extrabold text-slate-800">{assign.staffName}</span>
								{#if isSelf}
									<span
										class="py-0.2 rounded bg-indigo-600 px-1.5 text-[9px] font-extrabold text-white"
										>あなた</span
									>
								{/if}
							</div>
							<div
								class="flex items-center gap-1.5 font-mono text-[11px] font-extrabold text-slate-700"
							>
